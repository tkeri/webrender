...
void main(in a2v IN, out v2p OUT) {
    ...
    SHADER_OUT(vGradientRepeat, float(int(gradient.start_end_radius_ratio_xy_extend_mode.w) != EXTEND_MODE_CLAMP));
...
void main(in v2p IN, out p2f OUT) {
    ...
    SHADER_OUT(Target0, color * do_clip(vClipMaskUvBounds, vClipMaskUv));
}
#endif
