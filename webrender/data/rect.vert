 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
        ...
        #define TEXEL_FETCH(texture, sampler, position, lod, offset) texelFetch(sampler2D(texture, sampler), position + offset, lod)
    #else
        #define HIGHP_SAMPLER_FLOAT
        #define TEXEL_FETCH(texture, sampler, position, lod, offset) texelFetchOffset(sampler2D(texture, sampler), position, lod, offset)
    #endif
#else
    #define HIGHP_SAMPLER_FLOAT
    #define TEXEL_FETCH(texture, sampler, position, lod, offset) texelFetchOffset(sampler2D(texture, sampler), position, lod, offset)
    ...
    #define BINDING(setIndex, bindingIndex, attribute) layout(set = setIndex, binding = bindingIndex) attribute

#else
    #define LAYOUT(index, attribute) attribute
    #define BINDING(setIndex, bindingIndex, attribute) attribute
#endif
...
//======================================================================================
    ...
    LAYOUT(0, out vec4 Target0);
#endif

//======================================================================================
// Shared shader uniforms
//======================================================================================
BINDING(0, 3, uniform texture2D tColor0);
BINDING(0, 4, uniform sampler sColor0);
...
BINDING(0, 5, uniform texture2D tCacheA8);
BINDING(0, 6, uniform sampler sCacheA8);

BINDING(0, 7, uniform texture2D tCacheRGBA8);
BINDING(0, 8, uniform sampler sCacheRGBA8);

// An A8 target for standalone tasks that is available to all passes.
BINDING(0, 9, uniform texture2D tSharedCacheA8);
BINDING(0, 10, uniform sampler sSharedCacheA8);
...
BINDING(0, 11, uniform HIGHP_SAMPLER_FLOAT texture2D tResourceCache);
BINDING(0, 12, uniform HIGHP_SAMPLER_FLOAT sampler sResourceCache);

vec4[2] fetch_from_resource_cache_2_direct(ivec2 address) {
    return vec4[2](
        TEXEL_FETCH(tResourceCache, sResourceCache, address, 0, ivec2(0, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, address, 0, ivec2(1, 0))
    );
}
...
vec4[2] fetch_from_resource_cache_2(int address) {
    ...
    return vec4[2](
        TEXEL_FETCH(tResourceCache, sResourceCache, uv, 0, ivec2(0, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, uv, 0, ivec2(1, 0))
    );
}
...
BINDING(0, 13, uniform HIGHP_SAMPLER_FLOAT texture2D tLayers);
BINDING(0, 14, uniform HIGHP_SAMPLER_FLOAT sampler sLayers);
BINDING(0, 15, uniform HIGHP_SAMPLER_FLOAT texture2D tRenderTasks);
BINDING(0, 16, uniform HIGHP_SAMPLER_FLOAT sampler sRenderTasks);
...
vec4[8] fetch_from_resource_cache_8(int address) {
    ...
    return vec4[8](
        TEXEL_FETCH(tResourceCache, sResourceCache, uv, 0, ivec2(0, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, uv, 0, ivec2(1, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, uv, 0, ivec2(2, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, uv, 0, ivec2(3, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, uv, 0, ivec2(4, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, uv, 0, ivec2(5, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, uv, 0, ivec2(6, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, uv, 0, ivec2(7, 0))
    );
}
...
vec4[4] fetch_from_resource_cache_4_direct(ivec2 address) {
    return vec4[4](
        TEXEL_FETCH(tResourceCache, sResourceCache, address, 0, ivec2(0, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, address, 0, ivec2(1, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, address, 0, ivec2(2, 0)),
        TEXEL_FETCH(tResourceCache, sResourceCache, address, 0, ivec2(3, 0))
    );
}
...
vec4 fetch_from_resource_cache_1_direct(ivec2 address) {
    return texelFetch(sampler2D(tResourceCache, sResourceCache), address, 0);
}

vec4 fetch_from_resource_cache_1(int address) {
    ivec2 uv = get_resource_cache_uv(address);
    return texelFetch(sampler2D(tResourceCache, sResourceCache), uv, 0);
}
...
Layer fetch_layer(int index) {
    ...
    layer.transform[0] = TEXEL_FETCH(tLayers, sLayers, uv0, 0, ivec2(0, 0));
    layer.transform[1] = TEXEL_FETCH(tLayers, sLayers, uv0, 0, ivec2(1, 0));
    layer.transform[2] = TEXEL_FETCH(tLayers, sLayers, uv0, 0, ivec2(2, 0));
    layer.transform[3] = TEXEL_FETCH(tLayers, sLayers, uv0, 0, ivec2(3, 0));

    layer.inv_transform[0] = TEXEL_FETCH(tLayers, sLayers, uv0, 0, ivec2(4, 0));
    layer.inv_transform[1] = TEXEL_FETCH(tLayers, sLayers, uv0, 0, ivec2(5, 0));
    layer.inv_transform[2] = TEXEL_FETCH(tLayers, sLayers, uv0, 0, ivec2(6, 0));
    layer.inv_transform[3] = TEXEL_FETCH(tLayers, sLayers, uv0, 0, ivec2(7, 0));

    vec4 clip_rect = TEXEL_FETCH(tLayers, sLayers, uv1, 0, ivec2(0, 0));
...
RenderTaskData fetch_render_task(int index) {
    ...
    task.data0 = TEXEL_FETCH(tRenderTasks, sRenderTasks, uv, 0, ivec2(0, 0));
    task.data1 = TEXEL_FETCH(tRenderTasks, sRenderTasks, uv, 0, ivec2(1, 0));
    task.data2 = TEXEL_FETCH(tRenderTasks, sRenderTasks, uv, 0, ivec2(2, 0));
...
void write_clip(vec2 global_pos, ClipArea area) {
    vec2 texture_size = vec2(textureSize(sampler2D(tSharedCacheA8, sSharedCacheA8), 0).xy);
...
#ifdef WR_FEATURE_TRANSFORM
LAYOUT(7, varying vec3 vLocalPos);
#endif
